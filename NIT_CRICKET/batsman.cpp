...
int main()
{
    ...
    sleep(3);
    vector<int> fielder(4);
    for(int i=0;i<4;i++)fielder[i]=generate_client(PORT+i);
    struct pollfd fds[1];
    fds[0].fd=serv_nusfd;
    fds[0].events=POLLIN;
    while(1)
            ...
            while(1)
            {
                     ...
                     if(fds[0].revents & POLLIN)
                     {
                            ...
                            if(fd==-1)
                            {
                                  ...
                                  for(int i=0;i<4;i++)
                                  {
                                        if(send(fielder[i],&r,sizeof(r),0)<0)
                                        {
                                              perror("\nsend_to_fielder");
                                        }    
                                  }
                            ...
                            int num1=(buffer[0]-'0')*10+buffer[1];
                            int num2=(buffer[2]-'0')*10+buffer[3];
                            int ran=(rand()+num1+num2)%40;
                            //ran=4;
                            for(int i=0;i<4;i++)
                            {
                                   if(send(fielder[i],&ran,sizeof(ran),0)<0)
                                   {
                                        perror("\nsend_to_fielder");
                                   }    
                            }
