            ...
            if(server)
            {
                     ...
                     while(1)
                     {
                            ...
                            if(sc[0].revents & POLLIN)
                            {
                                   ...
                                   if(read>0)
                                   {
                                         for(int i=0;i<read;i++) if(buffer[i]=='n')server=false;
                                   }
                                   break;
                            }
                     }
                 ...
                 poll(sc,1,2000);
                  if(sc[0].revents & POLLIN)
                  {
                            ...
                            for(int i=0;i<read;i++)cout<<buffer[i];cout<<endl;
                             if(read>0)
                             {
                                         for(int i=0;i<read;i++) if(buffer[i]=='s')server=true;
                             }
                  } 
                  else 
                  {   
                         send(aclfd,"Change_this_output\n",20,0);
                         while(1)
                         {
                               poll(sac,1,2000);
