...
int main(int argc, char const* argv[])
{
   ...
   vector<string> services={"./s2","./s3","./s4"};
   vector<int> sfd(3);
   ...
   {
            FD_ZERO(&readfds);
            for(int i=0;i<3;i++)FD_SET(sfd[i],&readfds);
            max_sd=sfd[0];
            for(int i=1;i<3;i++)max_sd=max(max_sd,sfd[i]);
            ...
            for (int i = 0; i < 3; ++i) 
            {
                     if (FD_ISSET(sfd[i], &readfds)) 
                     {
                                   ...
                                   int nsfd= accept(sfd[i], (struct sockaddr *)&client_addr, &addr_len);
                                   if (nsfd < 0) 
                                   {
                                           perror("Accept error");
                                           exit(EXIT_FAILURE);
                                  }
                                  //printf("New connection on port %d: socket %d\n", ntohs(server_addr[i].sin_port), new_socket);
                                  int c=0;
                                  c=fork();
                                 if(c==0)
                                 {
                                          ...
                                          dup2(nsfd,0);
                                          cout<<"from server\n";
                                          execl(services[i].c_str(),services[i].c_str(),(char*)NULL );
                                         return 0;
                                  }
                                   close(nsfd);
                   }
